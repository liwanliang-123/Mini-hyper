...
{
    struct vgicv3_cpu *vgic_cpu = (struct vgicv3_cpu *)malloc(sizeof(struct vgicv3_cpu));
    if(vgic_cpu == NULL) {
...
static struct vgicv3_irq_config *vgic_irq_get(struct vcpu *vcpu, int irq_num)
{
    ...
    } else if(irq_num >= 16 && irq_num < 32) {
        return &vcpu->vgic_cpu->ppis[irq_num];
    } else if(irq_num >=32 ) {
        return &vcpu->vm->vgic_dist->spis[irq_num-32];
    } else {
...
static int vgicd_read(struct vcpu *vcpu, u64 offset, u64 *val, struct vmmio_access *vmmio)
{
    ...
    switch(offset) {
        ...
        case GICD_TYPER: {
            /* ITLinesNumber */
            u32 reg = ((vgic_dist->nspis + 32) >> 5) - 1;
            /* CPUNumber */
            reg |= (8 - 1) << GICD_TYPER_CPUNumber_SHIFT;
            *val = reg;
            goto finished;
        }
        ...
        case GICD_IGROUPR(0) ... GICD_IGROUPR(31) + 3: {
            u32 igrp = 0;
            irq_num = (offset - GICD_IGROUPR(0)) / sizeof(u32) * 32;
            for(int i = 0; i < 32; i++) {
                irq = vgic_irq_get(vcpu, irq_num + i);
                igrp |= (u32)irq->group << i;
            }
            *val = igrp;
            goto finished;
        }
        ...
        case GICD_IROUTER(32) ... GICD_IROUTER(1019) + 3:
            *val = 0;
            goto finished;
        case GICD_PIDR2:    /* Linux gic driver will read gicd_pidr2 */
...
{
    struct vgicv3_dist *vgic_dist = (struct vgicv3_dist *)malloc(sizeof(struct vgicv3_dist));
    if(vgic_dist == NULL) {
    ...
    vgic_dist->enabled = 0;
    vgic_dist->spis = (struct vgicv3_irq_config *)malloc(vgic_dist->nspis);
    if(vgic_dist->spis == NULL) {
        abort("Unable to alloc vgic_dist->spis, no memory");
    }

    /*
        所谓的 mmio 其实就是对要访问的地址没有做二阶段映射，当 VM 访问这些地址时，就会产生 data abort
        即没有对 GICD_BASE + GICD_SIZE 和 GICR_BASE + GICR_SIZE 进行二阶段映射
    */

    /* Set up virtual MMIO for gicd and gicr */
