...
int vmmio_handler(struct vcpu *vcpu, int reg_num, struct vmmio_access *vmmio)
{
    ...
    for(struct vmmio_info *m = vmmios; m != NULL; m = m->next) {
        ...
        if(m->base <= ipa && ipa < m->base + m->size) {
            if(vmmio->wnr) {
                if(m->vmmio_write) {
                    //LOG_INFO("[VMMIO WRITE]: device base: %p, offset is %p\n", m->base, ipa - m->base);
                    return m->vmmio_write(vcpu, ipa - m->base, val, vmmio);
                }
            } else {
                if(m->vmmio_read) {
                    //LOG_INFO("[VMMIO READ]: device base: %p, offset is %p\n", m->base, ipa - m->base);
                           ...
                           int (*vmmio_write)(struct vcpu *, u64, u64, struct vmmio_access *))
{
    ...
    struct vmmio_info *new = (struct vmmio_info *)malloc(sizeof(struct vmmio_info));
    new->next = vm->vmmios;
    vm->vmmios = new;
