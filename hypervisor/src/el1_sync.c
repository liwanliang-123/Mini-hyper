...
static int data_abort_handler(vcpu_t *vcpu, u64 esr_iss, u64 far)
{
    ...
    switch(sas) {
        case 0: accesz = VMMIO_BYTE; break;
        case 1: accesz = VMMIO_HALFWORD; break;
        case 2: accesz = VMMIO_WORD; break;
        case 3: accesz = VMMIO_DOUBLEWORD; break;
        default: abort("Unknow SAS from esr_el2");
    }

    struct vmmio_access vmmio;
    vmmio.ipa = ipa;
    vmmio.accsize = accesz;
    vmmio.wnr = wnr;

    if(vmmio_handler(vcpu, srt, &vmmio) < 0) {
...
void el1_sync_proc()
{
    ...
    switch(esr_ec) {
        ...
        case 0x16:
            ...
            LOG_INFO("\033[1;32m [el1_sync_proc] hvc trap from EL1\033[0m\n");
